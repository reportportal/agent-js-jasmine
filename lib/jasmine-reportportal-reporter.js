...
class ReportportalReporter {
  ...
  getHookStartTime(hookType, parent) {
    if (hookType === entityType.BEFORE_METHOD || hookType === entityType.BEFORE_SUITE) {
      const parentTimeAsNumber =
        (parent && SpecificUtils.convertIsoStringToMicroseconds(parent.startTime)) || 0;
      const hookStartTimeAsNumber = Math.max(
        parentTimeAsNumber,
        SpecificUtils.getBeforeHookStartTimeInMicroseconds(this.itemStartTime)
      );
      return clientHelpers.formatMicrosecondsToISOString(hookStartTimeAsNumber);
    }
  ...
  specDone(spec) {
    ...
    return promise.then((fileObj) => {
      const allLogs = SpecificUtils.getAllLogs(spec, fileObj, logs);
      const issue =
        status === RP_STATUSES.SKIPPED && this.conf.skippedIssue === false
          ? { issueType: 'NOT_ISSUE' }
          : null;
  ...
  suiteDone(suite) {
    ...
    const suiteDonePromise = this.client.finishTestItem(
      parent && parent.tempId,
