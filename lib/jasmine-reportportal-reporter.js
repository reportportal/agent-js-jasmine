...
class ReportportalReporter {
    ...
    addTestItemLog(testItemLog) {
        const logsWithTime = Object.assign(testItemLog.log, { time: this.getTime() });

        if (testItemLog && testItemLog.suite) {
            const logs = (this.suiteLogs.get(testItemLog.suite) || []).concat([logsWithTime]);

            this.suiteLogs.set(testItemLog.suite, logs);
        } else {
            const logs = this.additionalCustomParams.logs
                ? { logs: this.additionalCustomParams.logs.concat([logsWithTime]) }
                : { logs: [logsWithTime]};
