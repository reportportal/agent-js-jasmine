...
class ReportportalReporter {
    ...
    getParentId(number) {
        ...
        if ((this.parentIds.length - number) < 0) {
            return null;
        }

        return this.parentIds[this.parentIds.length - number];
    }
    ...
    hookStarted(hook) {
        const parent = this.getParentId(2);
    ...
    makeWrapperAll(wrapped, funcStart, funcFinish) {
        return function (action, timeout) {
            wrapped(function (done) {
                let isFuncBeCalled = true;

                try {
                    isFuncBeCalled = SpecificUtils.isHookShouldBeCalled(action());
    ...
    installHooks() {
        ...
        const wrapperBeforeAll =
            this.makeWrapperAll(jasmineBeforeAll, () => this.hookStarted(hookTypes.BEFORE_ALL), (status, err) => this.hookDone(hookTypes.BEFORE_ALL, status, err));
        const wrapperAfterAll =
            this.makeWrapperAll(jasmineAfterAll, () => this.hookStarted(hookTypes.AFTER_ALL), (status, err) => this.hookDone(hookTypes.AFTER_ALL, status, err));
        const wrapperBeforeEach =
            this.makeWrapperAll(jasmineBeforeEach, () => this.hookStarted(hookTypes.BEFORE_EACH), (status, err) => this.hookDone(hookTypes.BEFORE_EACH, status, err));
        const wrapperAfterEach =
            this.makeWrapperAll(jasmineAfterEach, () => this.hookStarted(hookTypes.AFTER_EACH), (status, err) => this.hookDone(hookTypes.AFTER_EACH, status, err));
