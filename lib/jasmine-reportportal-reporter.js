...
class ReportportalReporter {
    ...
    setStatus(data) {
        if (data && data.suite) {
            this.suiteStatus.set(data.suite, data.status);
        } else {
    ...
    getSuiteStatusBySuite(suite) {
        return this.suiteStatus.get(suite);
    }
    ...
    specDone(spec) {
        ...
        return promise.then((fileObj) => {
            ...
            const issue = (status === RPStatuses.SKIPPED && this.conf.skippedIssue === false) ? { issueType: 'NOT_ISSUE' } : null;
    ...
    suiteDone(suite) {
        ...
        this.finishParent();
        this.suiteStatus.delete(suite.description);
        this.startTime = null;
    }
