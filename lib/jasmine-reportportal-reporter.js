...
class ReportportalReporter {
    ...
    getParentInfo() {
        if (!this.parentInfo.length) {
            return null;
        }

        return this.parentInfo[this.parentInfo.length - 1];
    }

    getParentOfParentInfo() {
        if (this.parentInfo.length > 1) {
            return this.parentInfo[this.parentInfo.length - 2];
        }
    ...
    suiteStarted(suite) {
        ...
        return promise.then((codeRef) => {
            ...
            this.setParentInfo({ tempId: suiteObj.tempId, startTime: this.startTime });
    ...
    specStarted(spec) {
        ...
        return promise.then((codeRef) => {
            ...
            this.setParentInfo({ tempId: stepObj.tempId, startTime: this.startTime });
    ...
    getHookStartTime(hookType, parent) {
        if (hookType === entityType.BEFORE_METHOD || hookType === entityType.BEFORE_SUITE) {
            return Math.max(parent && parent.startTime, this.startTime - 1);
        }
    ...
    suiteDone(suite) {
        ...
        let suiteDonePromise = this.client.finishTestItem(parent && parent.tempId,
